...
export const Navbar = () => {
  ...
  return (
    <div className="Navbar w-screen h-[4rem] flex items-center justify-between absolute z-20">
      <div
        className="mx-[1rem] pl-[2rem] cursor-pointer"
        onClick={() => navigate(`/`)}
      >
      ...
      <div className="pr-[2rem]">
        <button className="border border-[#db0000]  text-[#db0000] font-semibold rounded-full p-[0.3rem] w-[6rem] hover:scale-110 duration-100">
          Login
        </button>
        <button
          className="border border-[#db0000]  text-white bg-[#db0000] font-semibold rounded-full p-[0.3rem] mr-[1rem] ml-[0.5rem] w-[6rem] hover:scale-110 duration-100"
          onClick={() => {
            dispatch(LogOut());
          }}
        >
