...
import { getDataPopularAll } from "../redux/actions/authMovie";
    ...
    <>
                ...
                <SwiperSlide key={value.id}>
                  ...
                  <div className="absolute inset-[-1px] h-100 bg-gradient-to-b from-transparent to-black top-1/2 transform">
                    ...
                    <div className="flex justify-center items-center mr-[0.5rem]">
                      ...
                      ></img>
                      <h6 className="text-white ml-[0.3rem] mr-[1rem] my-[1rem]">
                        {value.vote_average}/10
                      </h6>
                    </div>
                    <button
                      className="border border-red-700 text-white bg-red-700 font-semibold rounded-full px-[1rem] mr-[1rem] h-[2.5rem] hover:scale-110 duration-100"
      ...
      <div className="w-screen h-[42rem] bg-black pt-[5rem]">
        <div className="flex justify-between items-center pb-[2rem]">
          <h2 className="text-white text-[2.5rem] font-bold pl-[6rem]">
            Popular Movie
          </h2>
          <div
            className="text-[#db0000] pr-[6rem] text-[1rem] font-semibold cursor-pointer hover:scale-105"
        ...
        <div className="w-screen flex">
          {movies.slice(0, 4).map((value) => {
            return (
              <div
                className="flex flex-wrap w-[16rem] m-auto cursor-pointer"
                key={value.id}
                onClick={() => navigate(`/detail/${value.id}`)}
              >
                <img
                  ...
                  alt=""
                  className="w-[15rem] m-auto rounded-md hover:scale-105"
                />
                <div className="pt-[1rem] pl-[1rem] flex flex-col">
                  <h6 className="text-white text-[1.2rem] font-semibold">
                    {value.original_title}
                  </h6>
